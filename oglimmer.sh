...
DEFAULT_FRONTEND_DEPLOYMENT="start-renovate-frontend"
DEFAULT_BACKEND_DEPLOYMENT="start-renovate-backend"
...
# Restart hook configuration (used when kubectl is not available). The hook
# triggers an in-cluster rollout; see restart_deployment() below.
...
Build, deploy, and release start-renovate application components.
...
BUILD OPTIONS:
    ...
    --registries "REG1,REG2"    Comma-separated list of registries to push to (default: ${DEFAULT_REGISTRIES[0]})
                               Images will be tagged as REGISTRY/start-renovate-fe and REGISTRY/start-renovate-be
...
ENVIRONMENT VARIABLES:
    ...
    RESTART_TOKEN           Bearer token for the restart hook. Used to restart
                            deployments when kubectl is not available (e.g. CI).
...
parse_args() {
    ...
    if [[ ${#REGISTRIES[@]} -gt 0 ]]; then
        ...
        for registry in "${REGISTRIES[@]}"; do
            FRONTEND_IMAGES+=("$registry/start-renovate-fe")
            BACKEND_IMAGES+=("$registry/start-renovate-be")
        done
    else
        # Fallback to defaults if no registries specified
        FRONTEND_IMAGES=("${DEFAULT_REGISTRIES[0]}/start-renovate-fe")
        BACKEND_IMAGES=("${DEFAULT_REGISTRIES[0]}/start-renovate-be")
    fi
...
check_prerequisites() {
    ...
    # RESTART_TOKEN (to call the restart hook). Build-only environments like CI
    # have neither and pass --no-restart. Validate the requirement early so we
    # fail before building rather than after.
