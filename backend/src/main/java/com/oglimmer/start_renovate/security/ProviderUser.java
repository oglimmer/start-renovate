 ...
 * (GitHub exposes {@code login}; GitLab exposes {@code username}) into one shape the rest of the app
 * can consume. The provider's numeric user id is used as {@code providerUserId}; both GitHub and
 * GitLab registrations are configured with {@code id} as the user-name attribute, so {@link
...
public record ProviderUser(
  ...
  public static ProviderUser from(String provider, OAuth2User principal) {
    ...
    return new ProviderUser(
        provider,
        providerUserId,
        login != null ? login : providerUserId,
        name,
        avatarUrl);
  }
