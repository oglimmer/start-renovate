...
/**
 * A repository's Renovate config, fetched on behalf of the authenticated user so the SPA can load it
 * into the generator's editor. Unlike the public URL import, this goes through the user's OAuth token
 * and therefore works for private repos and for GitLab (whose raw endpoint blocks browser CORS).
 *
